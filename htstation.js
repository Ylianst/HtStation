...
const MqttReporter = require('./utils/MqttReporter.js');
...
// Enable auto-reconnection with 15-second interval
radio.setAutoReconnectEnabled(true);
...
function sendAprsPacket(aprsMessage) {
    ...
    try {
        ...
        const AX25PacketClass = require('./AX25Packet.js');
...
// === AX.25 Session Management ===
// Single active session (only one connection allowed at a time)
let activeSession = null;
let activeSessionCallsign = null;

// Create a parent object that mimics the C# MainForm interface for AX25Session
const sessionParent = {
    callsign: RADIO_CALLSIGN,
    stationId: RADIO_STATIONID,
    RADIO_CALLSIGN: RADIO_CALLSIGN,
    RADIO_STATIONID: RADIO_STATIONID,
    activeChannelIdLock: 1, // Assume channel 1 for now
    Debug: (msg) => console.log(`[Session Debug] ${msg}`)
};

// Function to create a new AX.25 session
function createAX25Session(remoteCallsign) {
    const session = new AX25Session(sessionParent, radio);
    
    // Set up event listeners
    session.on('stateChanged', (state) => {
        const stateNames = ['', 'DISCONNECTED', 'CONNECTED', 'CONNECTING', 'DISCONNECTING'];
        console.log(`[AX25Session] ${remoteCallsign} state changed to: ${stateNames[state]}`);
        
        // Publish session state to MQTT if enabled
        if (mqttReporter && config.MQTT_TOPIC) {
            const sessionStateTopic = `${config.MQTT_TOPIC}/ax25_session/${remoteCallsign.replace('-', '_')}/state`;
            mqttReporter.publishStatus(sessionStateTopic, { 
        ...
        if (state === AX25Session.ConnectionState.DISCONNECTED) {
            console.log(`[AX25Session] Session with ${remoteCallsign} disconnected, clearing active session`);
            if (activeSession === session) {
                activeSession = null;
                activeSessionCallsign = null;
            }
        } else if (state === AX25Session.ConnectionState.CONNECTED) {
            console.log(`[AX25Session] Session with ${remoteCallsign} connected - echo mode enabled`);
        }
    });
    
    session.on('dataReceived', (data) => {
        const message = data.toString('utf8');
        console.log(`[AX25Session] Data received from ${remoteCallsign}: "${message}"`);
        
        // Publish received data to MQTT if enabled
        if (mqttReporter && config.MQTT_TOPIC) {
            const sessionDataTopic = `${config.MQTT_TOPIC}/ax25_session/${remoteCallsign.replace('-', '_')}/data`;
            mqttReporter.publishStatus(sessionDataTopic, {
                message: message,
                sender: remoteCallsign,
                timestamp: new Date().toISOString()
            });
        }
        ...
        console.log(`[AX25Session] Echoed back to ${remoteCallsign}: "${message}"`);
    });
    
    session.on('uiDataReceived', (data) => {
        const message = data.toString('utf8');
        console.log(`[AX25Session] UI data received from ${remoteCallsign}: "${message}"`);
    });
    
    session.on('error', (error) => {
        console.error(`[AX25Session] Error in session with ${remoteCallsign}: ${error}`);
    });
    
    return session;
}

// Function to get or create a session for a remote station (single session only)
function getOrCreateSession(remoteCallsign) {
    const upperCallsign = remoteCallsign.toUpperCase();
    
    // If we already have an active session with a different station, reject this connection
    if (activeSession && activeSessionCallsign && activeSessionCallsign !== upperCallsign) {
        console.log(`[AX25Session] Rejecting connection from ${remoteCallsign} - already connected to ${activeSessionCallsign}`);
        return null;
    }
    
    // If no active session or it's the same station, create/return the session
    if (!activeSession) {
        console.log(`[AX25Session] Creating new session for ${remoteCallsign}`);
        activeSession = createAX25Session(remoteCallsign);
        activeSessionCallsign = upperCallsign;
    }
    
    return activeSession;
}

// Function to initiate an outgoing AX.25 connection
function connectToStation(destinationCallsign, destinationSSID = 0) {
    const AX25Address = require('./AX25Address.js');
    
    const remoteCallsign = `${destinationCallsign}-${destinationSSID}`;
    
    // Check if we already have an active session
    if (activeSession && activeSessionCallsign) {
        console.error(`[AX25Session] Cannot connect to ${remoteCallsign} - already connected to ${activeSessionCallsign}`);
        return null;
    }
    
    const session = getOrCreateSession(remoteCallsign);
    if (!session) {
        console.error(`[AX25Session] Failed to create session for ${remoteCallsign}`);
        return null;
    }
    
    // Create address list: [destination, source]
    const addresses = [
        new AX25Address(destinationCallsign, destinationSSID),
        new AX25Address(RADIO_CALLSIGN, RADIO_STATIONID)
    ];
    
    console.log(`[AX25Session] Initiating connection to ${remoteCallsign}`);
    
    const success = session.connect(addresses);
    if (!success) {
        console.error(`[AX25Session] Failed to initiate connection to ${remoteCallsign}`);
        return null;
    }
...
// Function to send data to the connected station
function sendToStation(message) {
    if (!activeSession) {
        console.error(`[AX25Session] No active session`);
        return false;
    }
    
    if (activeSession.currentState !== AX25Session.ConnectionState.CONNECTED) {
        console.error(`[AX25Session] Session is not connected (state: ${activeSession.currentState})`);
        return false;
    }
    
    activeSession.send(message);
    console.log(`[AX25Session] Sent message to ${activeSessionCallsign}: "${message}"`);
    return true;
}
...
radio.on('data', (frame) => {
    ...
    if (packet) {
        ...
        // Check if this is an AX.25 session packet (connection-oriented)
        const firstAddr = packet.addresses[0];
        const secondAddr = packet.addresses.length > 1 ? packet.addresses[1] : null;
        
        // Check if packet is addressed to our station
        if (firstAddr.address === RADIO_CALLSIGN && firstAddr.SSID === RADIO_STATIONID) {
            // This is an AX.25 session packet addressed to us
            if (secondAddr) {
                const senderCallsign = `${secondAddr.address}-${secondAddr.SSID}`;
                
                // Check if this is a connection request or packet for existing session
                if (packet.type === AX25Packet.FrameType.U_FRAME_SABM || 
                    packet.type === AX25Packet.FrameType.U_FRAME_SABME) {
                    
                    // Incoming connection request
                    console.log(`[AX25Session] Incoming connection request from ${senderCallsign}`);
                    
                    // Check if we can accept the connection (no active session or same station)
                    const session = getOrCreateSession(senderCallsign);
                    if (session) {
                        console.log(`[AX25Session] Accepting connection from ${senderCallsign}`);
                        session.receive(packet);
                    } else {
                        console.log(`[AX25Session] Rejecting connection from ${senderCallsign} - already busy`);
                        // Send DM (Disconnect Mode) response to reject the connection
                        const AX25PacketClass = require('./AX25Packet.js');
                        const rejectPacket = new AX25PacketClass(
                            [packet.addresses[1], packet.addresses[0]], // Flip addresses
                            0, 0, true, false, // nr, ns, pollFinal, command
                            AX25Packet.FrameType.U_FRAME_DM
                        );
                        rejectPacket.pid = packet.pid;
                        rejectPacket.channel_id = packet.channel_id;
                        rejectPacket.channel_name = packet.channel_name;
                        
                        const serialized = rejectPacket.ToByteArray ? rejectPacket.ToByteArray() : 
                                         (rejectPacket.toByteArray ? rejectPacket.toByteArray() : null);
                        if (serialized && typeof radio.sendTncFrame === 'function') {
                            radio.sendTncFrame({
                                channel_id: rejectPacket.channel_id,
                                data: serialized
                            });
                            console.log(`[AX25Session] Sent DM response to reject connection from ${senderCallsign}`);
                        }
                    }
                } else if (activeSession && activeSessionCallsign === senderCallsign.toUpperCase()) {
                    // Packet for existing session
                    console.log(`[AX25Session] Processing packet from active session ${senderCallsign}`);
                    activeSession.receive(packet);
                } else if (activeSession) {
                    // Packet from different station while we have an active session
                    console.log(`[AX25Session] Ignoring packet from ${senderCallsign} - active session with ${activeSessionCallsign}`);
                } else {
                    // Packet but no active session - send DM response
                    console.log(`[AX25Session] No active session for packet from ${senderCallsign} - sending DM`);
                    const AX25PacketClass = require('./AX25Packet.js');
                    const dmPacket = new AX25PacketClass(
                        [packet.addresses[1], packet.addresses[0]], // Flip addresses
                        0, 0, true, false, // nr, ns, pollFinal, command
                        AX25Packet.FrameType.U_FRAME_DM
                    );
                    dmPacket.pid = packet.pid;
                    dmPacket.channel_id = packet.channel_id;
                    dmPacket.channel_name = packet.channel_name;
                    
                    const serialized = dmPacket.ToByteArray ? dmPacket.ToByteArray() : 
                                     (dmPacket.toByteArray ? dmPacket.toByteArray() : null);
                    if (serialized && typeof radio.sendTncFrame === 'function') {
                        radio.sendTncFrame({
                            channel_id: dmPacket.channel_id,
                            data: serialized
                        });
                    }
                }
            ...
            // Packet not addressed to us - handle UI frames for APRS/monitoring only
            // (APRS handling is already done above, so we can ignore other packets)
            console.log(`[AX25Session] Packet not addressed to our station (${firstAddr.address}-${firstAddr.SSID})`);
        }
    } else {
