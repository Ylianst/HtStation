/*
Copyright 2026 Ylian Saint-Hilaire
Licensed under the Apache License, Version 2.0 (the "License");
http://www.apache.org/licenses/LICENSE-2.0
*/

const { SbcMode } = require('./SbcEnums');

/**
 * Pre-calculated tables and constants for SBC encoding/decoding
 */
const SbcTables = {
    /**
     * CRC-8 lookup table for header validation
     * Generator polynomial: G(X) = X^8 + X^4 + X^3 + X^2 + 1 (0x1d)
     */
    Crc8Table: new Uint8Array([
        
    ]),

    /**
     * Range scale factors for dequantization (fixed-point 1.28 format)
     * range_scale[i] = (1 << 28) / ((1 << (i+1)) - 1) for i in [0..15]
     */
    RangeScale: new Int32Array([
        0x0FFFFFFF, 0x05555556, 0x02492492, 0x01111111,
        0x00842108, 0x00410410, 0x00204081, 0x00101010,
        0x00080402, 0x00040100, 0x00020040, 0x00010010,
        0x00008004, 0x00004001, 0x00002000, 0x00001000
    ]),

    /**
     * Cosine values for 8-point DCT (fixed-point 0.13 format)
     * cos(i*pi/8) for i = [0..3]
     */
    Cos8: new Int16Array([8192, 7568, 5793, 3135]),

    /**
     * Cosine values for 16-point DCT (fixed-point 0.13 format)
     * cos(i*pi/16) for i = [0..7]
     */
    Cos16: new Int16Array([8192, 8035, 7568, 6811, 5793, 4551, 3135, 1598]),

    /**
     * Loudness offset for 4 subbands [frequency][subband]
     */
    LoudnessOffset4: [
        new Int32Array([-1, 0, 0, 0]),  // 16 kHz
        new Int32Array([-2, 0, 0, 1]),  // 32 kHz
        new Int32Array([-2, 0, 0, 1]),  // 44.1 kHz
        new Int32Array([-2, 0, 0, 1])   // 48 kHz
    ],

    /**
     * Loudness offset for 8 subbands [frequency][subband]
     */
    LoudnessOffset8: [
        new Int32Array([-2, 0, 0, 0, 0, 0, 0, 1]),  // 16 kHz
        new Int32Array([-3, 0, 0, 0, 0, 0, 1, 2]),  // 32 kHz
        new Int32Array([-4, 0, 0, 0, 0, 0, 1, 2]),  // 44.1 kHz
        new Int32Array([-4, 0, 0, 0, 0, 0, 1, 2])   // 48 kHz
    ],

    /**
     * Saturate value to 16-bit signed range
     * @param {number} value - Value to saturate
     * @returns {number} Saturated value
     */
    saturate16(value) {
        if (value > 32767) return 32767;
        if (value < -32768) return -32768;
        return value | 0;
    },

    /**
     * Count leading zeros in a 32-bit unsigned integer
     * @param {number} value - Value to count leading zeros
     * @returns {number} Number of leading zeros
     */
    countLeadingZeros(value) {
        value = value >>> 0; // Convert to unsigned 32-bit
        if (value === 0) return 32;

        let count = 0;
        if ((value & 0xFFFF0000) === 0) { count += 16; value <<= 16; }
        if ((value & 0xFF000000) === 0) { count += 8; value <<= 8; }
        if ((value & 0xF0000000) === 0) { count += 4; value <<= 4; }
        if ((value & 0xC0000000) === 0) { count += 2; value <<= 2; }
        if ((value & 0x80000000) === 0) { count += 1; }

        return count;
    },

    /**
     * Compute CRC-8 of frame data
     * @param {Object} frame - SBC frame object
     * @param {Uint8Array} data - Frame data
     * @param {number} size - Data size
     * @returns {number} CRC value or -1 on error
     */
    computeCrc(frame, data, size) {
        const SbcFrame = require('./SbcFrame');
        const nch = frame.mode !== SbcMode.Mono ? 2 : 1;
        const nsb = frame.subbands;
        const nbit = nch * nsb * 4 + (frame.mode === SbcMode.JointStereo ? nsb : 0);

        if (size < ((SbcFrame.HEADER_SIZE * 8 + nbit + 7) >> 3)) {
            return -1;
        }

        let crc = 0x0f;
        crc = this.Crc8Table[crc ^ data[1]];
        crc = this.Crc8Table[crc ^ data[2]];

        for (let i = 4; i < 4 + Math.floor(nbit / 8); i++) {
            crc = this.Crc8Table[crc ^ data[i]];
        }

        if (nbit % 8 !== 0) {
            crc = ((crc << 4) ^ this.Crc8Table[(crc >> 4) ^ (data[4 + Math.floor(nbit / 8)] >> 4)]) & 0xFF;
        }

        return crc;
    }
};

module.exports = SbcTables;
